...
| **`ProductDetails.js`** | `<PriceBlock product={product} mode="details" price={currentPrice} comparePrice={comparePrice} size="lg" taxNote=… />` ([:360–367](../src/pages/ProductDetails/ProductDetails.js)) — tier table renders; explicit variant price drives exact mode; enquiry-safe note kept (suppressed on onEnquiry). | ✅ |
...
| **CSS Modules + palette separation** | all new styles `--sf-*`-driven; admin palette untouched | ✅ |
